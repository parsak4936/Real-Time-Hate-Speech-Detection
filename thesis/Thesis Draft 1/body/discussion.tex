...
\textbf{Latency and cost.} Every gain from the second tier is bought with LLM calls that take seconds, and the multi-agent variant spends four of them per message. The tiered design keeps that off the live path, but the cost is real and would shape any production deployment. On the fast path the dominant cost is not the classifier at all but the write to storage, about 47 milliseconds against 32 for the model (Section~\ref{throughput-scalability}).
