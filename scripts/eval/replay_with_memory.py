    ...
    for doc_id, source in _iter_target_records(es, args.csv, args.force):
        ...
        try:
            ...
            if getattr(exc, "status_code", None) == 429:
                print(f"\n[STOP] Ollama rate limit (429) after {processed} rows. "
                      f"Verdicts already written to ES are safe.")
                print("Re-run the SAME command later — finished rows are skipped automatically.")
                return
            print(f"\n[ERROR] Unexpected failure after {processed} rows: {exc}")
            raise
