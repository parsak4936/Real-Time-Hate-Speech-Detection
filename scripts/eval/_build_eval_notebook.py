...
code("""
    ...
    # or from notebooks/.
    def _find_benchmark():
        # Prefer the current multi-platform eval set; fall back to the retired
        # internship benchmark only if the new one is absent.
        for candidate in (
            "thesis_benchmark_eval.csv", "../thesis_benchmark_eval.csv",
            "thesis_final_benchmark.csv", "../thesis_final_benchmark.csv",
        ):
...
# ============================================================================
md("## 6. Multi-Agent (Tier-2 split into specialist agents)")
md(
    "Stage 4 replaces the single Tier-2 judge with four specialist agents — "
    "Risk Scorer, Behavior Profiler, Escalator, Supervisor — run in sequence. "
    "The Supervisor emits the final verdict in the same space as the other "
    "variants so it is directly comparable. The intermediate signals "
    "(risk score, behaviour risk, escalation action) are also stored, which "
    "is itself thesis-worthy explainability data.\n\n"
    "**Prerequisite — populate `agent_final_decision_with_multi_agent`:**\n"
    "```bash\n"
    "python -u scripts/replay_with_multi_agent.py --csv thesis_benchmark_eval.csv --limit 5   # test first\n"
    "python -u scripts/replay_with_multi_agent.py --csv thesis_benchmark_eval.csv --apply\n"
    "```"
)

md("### 6.1 Load and check for multi-agent verdicts")

code("""
    df_ma = load_benchmark()
    col_ma = "agent_final_decision_with_multi_agent"
    has_ma = col_ma in df_ma.columns and df_ma[col_ma].astype(str).str.strip().replace("nan", "").ne("").any()

    if not has_ma:
        print("Multi-agent verdicts NOT FOUND. Run scripts/replay_with_multi_agent.py first.")
    else:
        df_ma[col_ma] = df_ma[col_ma].astype(str).str.strip().str.lower()
        df_ma["agent_pred_baseline"] = df_ma.apply(
            lambda r: agent_effective_label(r["model_label"], r["agent_final_decision"]),
            axis=1,
        )
        df_ma["agent_pred_ma"] = df_ma.apply(
            lambda r: agent_effective_label(r["model_label"], r[col_ma]),
            axis=1,
        )
        # restrict to rows that actually have a multi-agent verdict
        df_ma = df_ma[df_ma[col_ma].ne("") & df_ma[col_ma].ne("nan")]
        print(f"Multi-agent verdicts present on {len(df_ma)} records.")
""")

md("### 6.2 Accuracy delta — baseline vs multi-agent vs Human")

code("""
    if has_ma:
        base_3 = compute_three_class_accuracy(df_ma["agent_pred_baseline"], df_ma["human_ground_truth"])
        ma_3   = compute_three_class_accuracy(df_ma["agent_pred_ma"],       df_ma["human_ground_truth"])
        base_b = compute_binary_accuracy(df_ma["agent_pred_baseline"], df_ma["human_ground_truth"])
        ma_b   = compute_binary_accuracy(df_ma["agent_pred_ma"],       df_ma["human_ground_truth"])
        print(pd.Series({
            "Baseline 3-class":                f"{base_3:.1%}",
            "Multi-agent 3-class":             f"{ma_3:.1%}",
            "Delta 3-class":                   f"{(ma_3 - base_3) * 100:+.2f} pp",
            "Baseline binary (toxic/normal)":  f"{base_b:.1%}",
            "Multi-agent binary":              f"{ma_b:.1%}",
            "Delta binary":                    f"{(ma_b - base_b) * 100:+.2f} pp",
        }).to_string())
    else:
        print("Skipped — see §6.1.")
""")

md("### 6.3 McNemar's test — is the multi-agent delta significant?")

code("""
    if has_ma:
        base_ok = df_ma["agent_pred_baseline"] == df_ma["human_ground_truth"]
        ma_ok   = df_ma["agent_pred_ma"]       == df_ma["human_ground_truth"]
        hurt   = int((base_ok & ~ma_ok).sum())
        helped = int((~base_ok & ma_ok).sum())
        print(f"multi-agent hurt: {hurt} | multi-agent helped: {helped}")
        if hurt + helped == 0:
            print("No discordant pairs — verdicts identical.")
        else:
            try:
                from scipy.stats import chi2
                stat = (abs(helped - hurt) - 1) ** 2 / (helped + hurt)
                p = float(chi2.sf(stat, df=1))
                print(f"McNemar chi^2={stat:.3f}, p={p:.4f} =>", "significant" if p < 0.05 else "NOT significant", "at 0.05")
            except ImportError:
                print("scipy not installed — pip install scipy for the p-value.")
    else:
        print("Skipped — see §6.1.")
""")

md("### 6.4 Specialist-signal distributions (explainability)")
md("How the internal agents behaved — risk scores, behaviour-risk labels, escalation routing.")

code("""
    if has_ma and "agent_ma_risk_score" in df_ma.columns:
        print("Risk score distribution:")
        print(pd.to_numeric(df_ma["agent_ma_risk_score"], errors="coerce").describe().to_string())
        print("\\nBehaviour-risk labels:")
        print(df_ma["agent_ma_behavior_risk"].value_counts().to_string())
        print("\\nEscalation routing:")
        print(df_ma["agent_ma_escalation_action"].value_counts().to_string())
        human_pct = (df_ma["agent_ma_escalation_action"].astype(str).str.lower() == "human_review").mean()
        print(f"\\nFraction routed to human review: {human_pct:.1%}")
    else:
        print("Skipped — signal columns not present.")
""")

md("### 6.5 Register the multi-agent variant")

code("""
    if has_ma:
        t1_ma = pd.to_numeric(df_ma["processing_time_ms"], errors="coerce").dropna().mean()
        t2_ma = (pd.to_numeric(df_ma["agent_latency_seconds"], errors="coerce").dropna() * 1000).mean()
        RESULTS_REGISTRY["Tier-1 + Multi-Agent (Stage 4)"] = {
            "accuracy_3class":  ma_3,
            "accuracy_binary":  ma_b,
            "mean_latency_ms":  (t1_ma or 0) + (t2_ma or 0),
            "n_records":        len(df_ma),
            "notes":            "4 specialist agents: risk scorer + behavior profiler + escalator + supervisor",
        }
        pd.DataFrame(RESULTS_REGISTRY).T
    else:
        print("Skipped — see §6.1.")
...
out_path = pathlib.Path(__file__).resolve().parents[2] / "notebooks" / "evaluation.ipynb"
